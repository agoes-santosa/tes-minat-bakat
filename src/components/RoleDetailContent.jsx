...
// Shared by RoleDetailPage (full page) and RoleModal (popup) so the two
// entry points to the same information stay in sync automatically.
export default function RoleDetailContent({ occupation }) {
  ...
  if (!field) {
    return (
      <p style={{ color: 'var(--color-text-muted)' }}>Informasi jurusan untuk peran ini belum tersedia.</p>
    );
  }
    ...
    <>
      ...
      {outlook && (
        <div className="outlook-section">
          ...
          <span className={`outlook-badge outlook-${outlook.tier}`}>{OUTLOOK_TIERS[outlook.tier].label}</span>
          <p style={{ marginTop: 10 }}>{outlook.summary}</p>
          <p className="outlook-tip">
            <strong>Cara beradaptasi:</strong> {outlook.tip}
          </p>
          <p className="form-hint">
            Perkiraan arah umum berdasarkan tren otomasi/AI dan pasar kerja Indonesia saat ini, bukan ramalan
            pasti — kondisi bisa berubah.
          </p>
        </div>
      )}
      ...
      <div className="outlook-section">
        ...
        <p className="form-hint">
          ...
          fakultas bersifat indikatif dan dapat berubah seiring reorganisasi kampus — cek situs resmi universitas
          untuk memastikan. Penjurusan SMA IPA/IPS/Bahasa juga sudah tidak wajib di banyak sekolah sejak Kurikulum
          Merdeka, jadi anggap ini sebagai panduan umum, bukan syarat mutlak.
        </p>
      </div>
    </>
  );
